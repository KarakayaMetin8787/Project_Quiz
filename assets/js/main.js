let data=[
    ...
    const dataList = [...data];
    const idContent = document.body.querySelector("#content")

    dataList.forEach((elt) => {
        let imageContainer = document.createElement("div");
        idContent.append(imageContainer);

        let images = document.createElement("img");
        images.setAttribute("src", `${elt.url}`);
        imageContainer.append(images);

        let questionTag = document.createElement("p");
        questionTag.textContent = elt.question;
        idContent.append(questionTag);

        let buttonContainer = document.createElement("div");
        idContent.append(buttonContainer);

        elt.choice.map((alt) => {
            let buttonsCreate= document.createElement("button");
            buttonsCreate.textContent = alt;
            buttonContainer.append(buttonsCreate);
        })}
    );
let counter = 0;
const allButtons = document.body.querySelectorAll("button")

    allButtons.forEach((olt) => {
        olt.addEventListener("click",() => {
            if (olt.textContent === String(dataList[counter].answer)) {
                olt.style.backgroundColor = "green";
                counter++;
            } else {
                olt.style.backgroundColor = "red";
        }});
    });
